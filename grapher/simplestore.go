...
func (self *SimpleGraphStore) GetMutationsAscending(perma_blobref string, entity_blobref string, field string, startWithSeqNumber int64, endSeqNumber int64) (ch <-chan map[string]interface{}, err os.Error) {
  g, ok := self.graphs[perma_blobref]
  if !ok {
    return nil, os.NewError("Unknown perma blob")
  }
  ...
  f := func() {
    for _, data := range g.nodes[startWithSeqNumber: endSeqNumber] {
      if data["k"].(int64) == OTNode_Mutation && data["e"].(string) == entity_blobref && data["f"].(string) == field {
	c <- data
      }
    }
    close(c)
